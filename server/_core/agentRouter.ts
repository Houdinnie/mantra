  ...
  for (const skill of VENTUREMIND_SKILLS) {
    if (lower.startsWith(skill.invoke)) {
      ...
      return {
        ...
        tier,
        cleanedMessage:
          trimmed.slice(skill.invoke.length).trim() ||
...
export function detectPersona(
  ...
  const contextWindow = [message, ...history.slice(-4).map(m => m.content)]
    .join(" ")
    .toLowerCase();
  ...
  for (const persona of PERSONAS) {
    const score = persona.triggers.reduce(
      (acc, t) => acc + (contextWindow.includes(t) ? 1 : 0),
      0
    );
    if (score > bestScore) {
      bestScore = score;
  ...
  return (
    bestMatch ??
    ({ id: "default", name: "Mantra", emoji: "🤖" } as unknown as Persona)
  );
}
...
export function routeMessage(
  ...
  const { mentionedPersona, cleanedMessage: mentionCleaned } =
    parseAgentMention(skillMatch.cleanedMessage);
  ...
  const needsSearch =
    skillMatch.routeType === "web_search" ||
    /\b(latest|current|today|this week|news|recent|2024|2025|2026|market cap|stock price|exchange rate)\b/i.test(
      rawMessage
    );
