...
export async function runMillionDollarIdea(context: IdeaContext): Promise<MillionDollarResult> {
  const contextStr = [
  ...
  const rawIdeasJson = await callClaude({
    ...
    messages: [{ role: "user", content: `Generate 5 business ideas for a founder with this context:\n\n${contextStr}\n\nReturn only the JSON array.` }],
    maxTokens: 2000,
  });

  let rawIdeas: Array<Omit<BusinessIdea, "cfoScore" | "cmoScore" | "overallScore" | "firstAction">> = [];
  try {
  ...
  const ideasSummary = rawIdeas.map((idea, i) =>
  ...
  const [cfoJson, cmoJson] = await Promise.all([
    callClaude({
      ...
      messages: [{ role: "user", content: `Score these ideas:\n${ideasSummary}\n\nReturn only JSON array.` }],
      maxTokens: 500,
    }),
    callClaude({
  ...
  try { cfoScores = JSON.parse(cfoJson.replace(/```json|```/g, "").trim()); } catch {}
  try { cmoScores = JSON.parse(cmoJson.replace(/```json|```/g, "").trim()); } catch {}
  ...
  const topIdeasJson = await callClaude({
    ...
    messages: [{ role: "user", content: `Rank and refine these scored ideas into the top 3. Add firstAction and overallScore to each:\n\n${synthesisInput}\n\nReturn only JSON array of 3.` }],
    maxTokens: 2000,
  });
  ...
  let topIdeaValidation = {
    ...
    whitespace: "",
    validationStep: topIdea?.firstAction ?? "Find 3 people with this problem and offer to solve it manually",
