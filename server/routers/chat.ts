  ...
  app.post("/api/chat/stream", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (routing.routeType === "board_session") {
        send({
          type: "status",
          text: "Convening your C-suite board session...",
        });
        const role = detectExecutiveRole(routing.userMessage);
        const exec = EXECUTIVES[role];
        send({
          type: "status",
          text: `Delegating to ${exec.emoji} ${exec.title} and colleagues...`,
        });
        ...
        send({
          type: "status",
          text: "📣 Franklin Marketing Agent activated — researching your market...",
        });
        ...
        send({
          type: "status",
          text: "📊 Franklin Trading Agent activated — researching markets...",
        });
        const research = await runTradingAgent({
          asset: routing.userMessage,
          context: routing.userMessage,
        });
        ...
        if (!urls.length) {
          ...
          send({
            type: "status",
            text: `📥 Ingesting ${urls.length} URL${urls.length > 1 ? "s" : ""} via DeepReeder...`,
          });
          const online = await isGatewayOnline();
          if (!online) {
...
${
  l?.top_concepts?.length
    ? `### Top Concepts Learned\n${l.top_concepts
        .slice(0, 5)
        .map(c => `- **${c.concept}** (×${c.count})`)
        .join("\n")}`
    : ""
}

${
  i?.cross_references?.length
    ? `### Cross-References Found\n${i.cross_references
        .slice(0, 3)
        .map(r => `- "${r.a}" ↔ "${r.b}" (strength: ${r.strength.toFixed(2)})`)
        .join("\n")}`
    : ""
}`;
        }
        ...
        send({
          type: "status",
          text: `🧠 Recalling memories for: "${query}"...`,
        });
        ...
        send({
          type: "status",
          text: "💡 Convening board ideation session — CEO, CFO, and CMO thinking...",
        });
        ...
        const ideaLines = result.ideas
          .map((idea, i) => {
            const rank = i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉";
            const complexity = {
              weekend: "⚡ Weekend build",
              "1-month": "📅 1-month build",
              "3-months": "🏗️ 3-month build",
...
${validationSection}`;
        ...
        const messages = [
          ...prior,
          { role: "user" as const, content: routing.userMessage },
        ];
        const stream = streamClaudeWithSearch({
          system: systemPrompt,
          messages,
        });
        for await (const event of stream) {
          if (event.type === "delta") {
            ...
            send({ type: "status", text: `🔍 Searching: ${event.query}` });
          } else if (event.type === "source") {
            send({ type: "source", url: event.url, title: event.title });
