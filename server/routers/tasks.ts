...
export const taskRouter = router({
  ...
  send: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const flow = createEmbeddedFlow({
        name: `Task: ${input.task.slice(0, 50)}`,
        trigger: "manual",
        steps: [{ id: "run", type: "message", content: input.task }],
        enabled: true,
      });
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!online)
        throw new TRPCError({
          ...
          message: "Direct tool invoke requires OpenClaw gateway",
        });
  ...
  createFlow: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const online = await isGatewayOnline();
      if (online) {
        const r = await createFlow(input as FlowDefinition);
        if (r) return r;
      }
      return createEmbeddedFlow({
        name: input.name,
  ...
  updateFlow: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const online = await isGatewayOnline();
      if (online) {
        const r = await updateFlow(input.id, input.updates as any);
        if (r) return r;
      }
  ...
  triggerFlow: protectedProcedure
    .input(
      z.object({ id: z.string(), input: z.record(z.unknown()).optional() })
    )
    .mutation(async ({ ctx, input }) => {
      if (!ctx.user) throw new TRPCError({ code: "UNAUTHORIZED" });
      const online = await isGatewayOnline();
      if (online) {
        const r = await triggerFlow(input.id, input.input);
        if (r) return r;
      }
      const runId = await triggerEmbeddedFlow(
        input.id,
  ...
  getFlowRun: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return (
        getEmbeddedLedger(500).find((e: any) => e.id === input.runId) ?? null
      );
    }),
  ...
  deepRead: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const online = await isGatewayOnline();
      if (online) {
        const r = await deepRead(input.urls, input.sessionKey);
        if (r) return r;
      }
      const flow = createEmbeddedFlow({
        name: `DeepRead: ${input.urls.length} URLs`,
        trigger: "manual",
        steps: input.urls.map((url, i) => ({
          id: `read_${i}`,
          type: "message" as const,
          content: `Fetch and summarise key content from: ${url}`,
          onSuccess: i < input.urls.length - 1 ? `read_${i + 1}` : undefined,
        })),
        enabled: true,
      });
      const runId = await triggerEmbeddedFlow(flow.id);
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      switch (input.template) {
        case "morningBriefing":
          fd = TASK_TEMPLATES.morningBriefing(p.channels ?? ["main"]);
          break;
        case "urlMonitor":
          fd = TASK_TEMPLATES.urlMonitor(p.url, p.checkEveryHours ?? 6);
          break;
        case "researchTask":
          fd = TASK_TEMPLATES.researchTask(p.topic);
          break;
        case "tradingAlert":
          fd = TASK_TEMPLATES.tradingAlert(p.asset, p.cronExpr);
          break;
      }
      const online = await isGatewayOnline();
      if (online) {
        const r = await createFlow(fd!);
        if (r) return r;
      }
      return createEmbeddedFlow({
        name: fd!.name,
        description: fd!.description,
        trigger: fd!.trigger as any,
        cronExpression: fd!.cronExpression,
        steps: fd!.steps.map(s => ({
          id: s.id,
          type: s.type as any,
          content: s.content,
          waitMs: s.waitMs,
