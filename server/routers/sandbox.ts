...
export async function registerSandboxStream(app: Express) {
  ...
  app.post("/api/sandbox/upload", async (req: Request, res: Response) => {
    ...
    if (!sessionId || !userId || !filename) {
      res.status(400).json({
        error: "x-session-id, x-user-id, x-filename headers required",
      });
      return;
    }
    ...
    if (!sandbox) {
      res
        .status(404)
        .json({ error: "Sandbox not found — run a task first to initialise" });
      return;
    }
    ...
    try {
      ...
      await writeFileInSandbox(
        sandbox,
        safeFilename,
        fileBuffer.toString("utf8").replace(/\0/g, "")
      );
      ...
      if (fileBuffer.includes(0)) {
        ...
        await execInSandbox(
          sandbox,
          `echo '${b64}' | base64 -d > /workspace/${safeFilename}`
        );
      }
