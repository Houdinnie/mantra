...
import { callExecutive, runBoardSession, detectExecutiveRole, type ExecutiveRole } from "../_core/clawCompany";
...
export const executiveRouter = router({
  ...
  boardSession: protectedProcedure
    .input(z.object({
      task: z.string().min(1),
      roles: z.array(z.enum(["ceo", "cfo", "cto", "coo", "cmo", "cso"])).default(["cfo", "cto", "coo"]),
      history: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string() })).default([]),
    }))
    .mutation(async ({ ctx, input }) => {
      if (!ctx.user) throw new TRPCError({ code: "UNAUTHORIZED" });
      try {
        return await runBoardSession(input.task, input.history, input.roles as ExecutiveRole[]);
      } catch (error) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: String(error) });
      }
    }),
...
export const franklinRouter = router({
  ...
  content: protectedProcedure
    .input(z.object({
      format: z.enum(["blog_post", "twitter_thread", "linkedin_post", "video_script", "email_sequence"]),
