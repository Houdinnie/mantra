...
export async function getUserStats(userId: number) {
  ...
  // O(N) -> O(1) Optimization: Use database SQL aggregation functions (count, sum, max)
  // to avoid loading thousands of session records into memory and computing aggregates.
  // This drastically reduces network payloads, memory usage, and execution time.
  const result = await db
    .select({
      totalSessions: count(),
  ...
  const row = result[0];
  const totalSessions = row?.totalSessions ? Number(row.totalSessions) : 0;
  const totalMessages = row?.totalMessages ? Number(row.totalMessages) : 0;
