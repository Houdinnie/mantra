...
export async function getUserStats(userId: number) {
  ...
  // Optimization: Use SQL aggregate functions (count, sum, max) to compute stats in database.
  // Reduces data transfer and memory footprint from O(N) to O(1) and avoids in-memory array sorting.
  ...
  return {
    totalSessions: stats?.totalSessions ? Number(stats.totalSessions) : 0,
