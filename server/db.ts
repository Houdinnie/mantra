...
export async function getUserStats(userId: number) {
  ...
  // Optimize: query with SQL aggregate functions directly instead of fetching all rows and calculating in-memory.
  // This avoids O(N) database transfer and memory overhead, reducing execution to O(1) in application space.
  const [result] = await db
    .select({
      totalSessions: count(chatSessions.sessionId),
  ...
  const lastActiveVal = result.lastActive;
  return {
