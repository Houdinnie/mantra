...
export async function getUserStats(userId: number) {
  ...
  // Optimized: Use SQL aggregates to avoid fetching all rows and aggregating in memory
  // This changes performance from O(N) to O(1) in terms of data transfer and memory usage.
  const [result] = await db
    .select({
      totalSessions: count(),
