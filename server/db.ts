...
/**
 * Optimized user session metrics aggregation using SQL aggregate functions.
 * Replaces O(N) memory allocation and O(N log N) in-memory array sorting
 * with O(1) SQL database aggregation (count, sum, max).
 */
export async function getUserStats(userId: number) {
  ...
  const [result] = await db
    .select({
      ...
      maxLastActive: max(chatSessions.updatedAt),
    })
    .from(chatSessions)
  ...
  const lastActive = result?.maxLastActive ? new Date(result.maxLastActive) : null;
