...
// Replaced loading all session rows into Node.js memory and performing JS reduce/sort (O(N) time & memory)
// with SQL aggregate queries using `count`, `sum`, and `max` (O(1) transfer overhead & DB-level index efficiency).
