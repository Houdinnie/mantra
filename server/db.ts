 ...
 * Replaced O(N) client-side array aggregation (fetching all session rows, summing message counts,
 * and sorting in memory) with O(1) database SQL aggregation (`count()`, `sum()`, `max()`).
 * This dramatically reduces database I/O, network bandwidth, memory allocation, and CPU execution time
 * for users with many chat sessions.
 */
