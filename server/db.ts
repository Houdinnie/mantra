...
export async function getUserStats(userId: number) {
  ...
  // Optimized database metrics aggregation:
  // Instead of fetching all chatSession rows in memory and reducing/sorting (O(N)),
  // we perform a native Drizzle SQL aggregation query directly in MySQL (O(1)).
  ...
  const totalSessions = stats?.totalSessions ? Number(stats.totalSessions) : 0;
  const totalMessages = stats?.totalMessages ? Number(stats.totalMessages) : 0;
