...
/**
 * Sets the internal database instance (used primarily for unit testing).
 */
...
/**
 * Retrieves user chat statistics.
 * Optimization: Uses SQL aggregate functions (count, sum, max) to calculate stats
 * at database level rather than fetching all chat sessions into memory and sorting/reducing.
 * Complexity: O(N) -> O(1) bandwidth/memory transfer, avoiding client-side array sorting O(N log N).
 */
