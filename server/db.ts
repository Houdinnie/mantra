...
// ⚡ Bolt Optimization: Optimized to use SQL aggregate functions (count, sum, max)
// from drizzle-orm to avoid fetching all rows and aggregating in memory, providing an O(N) to O(1) performance win.
export async function getUserStats(userId: number) {
  ...
  // Database results from aggregate functions like max(updatedAt) may be returned as ISO strings or Date objects depending on the driver; always wrap in new Date() for consistent usage in the application logic.
