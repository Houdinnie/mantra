...
export async function getUserStats(userId: number) {
  ...
  // ⚡ BOLT OPTIMIZATION: Compute stats inside the database engine using SQL aggregations (count, sum, max).
  // Complexity: O(N) database data transfer and memory usage, followed by O(N log N) in-memory sort is optimized to O(1) in-memory space and O(1) CPU.
  // This avoids fetching all rows for users with large number of sessions and processing them inside Node.js.
  ...
  return {
    ...
    // Ensure consistent Date object conversion across different SQL drivers/mock formats
