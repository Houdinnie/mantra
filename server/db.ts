...
// Optimization: Use SQL aggregate functions (count, sum, max) instead of fetching all rows into memory and sorting in Node.js.
// Reduces memory allocation and converts O(N) JS aggregation/sorting to O(1) SQL database query.
export async function getUserStats(userId: number) {
  ...
  if (!stats) return { totalSessions: 0, totalMessages: 0, lastActive: null };
