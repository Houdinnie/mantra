...
/**
 * Sets or overrides the active database connection instance (primarily for unit testing dependency injection).
 */
...
/**
 * Retrieves aggregated user metrics (total sessions, total messages, last active timestamp).
 *
 * PERFORMANCE OPTIMIZATION:
 * Replaced in-memory row fetching and array operations with SQL aggregation helpers (count, sum, max).
 * - Previous: Fetched all N user sessions into Node.js memory, calculated sum via array.reduce,
 *   and sorted array in place to find latest timestamp -> O(N) memory & O(N log N) JS execution time.
 * - Optimized: Pushes aggregation to database engine via single SQL query -> O(1) memory & application time.
 */
