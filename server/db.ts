...
export async function getUserStats(userId: number) {
  ...
  // ⚡ Bolt: Optimize by running a single aggregate query on the database instead of loading all sessions in memory (O(N) to O(1)).
  // This reduces memory pressure, database load, and network payload size by only returning exactly 1 calculated row.
  ...
  if (!result || result.length === 0 || Number(result[0].totalSessions) === 0) {
