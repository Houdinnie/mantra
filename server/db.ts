...
export async function getUserStats(userId: number) {
  ...
  // Optimization: Use SQL aggregate functions (count, sum, max) from drizzle-orm to
  // calculate metrics at the database level rather than fetching all rows and
  // aggregating in memory. This reduces complexity from O(N) memory and runtime to O(1).
