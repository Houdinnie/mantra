...
export async function getUserStats(userId: number) {
  ...
  // OPTIMIZATION: Instead of loading all session records into Node.js memory and doing in-memory
  // aggregation (O(N) database transfer/memory), we query using SQL aggregations (O(1) payload & memory).
  ...
  if (result.length === 0 || !result[0].totalSessions) {
