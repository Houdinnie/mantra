...
export async function getUserStats(userId: number) {
  ...
  // Optimize: use SQL aggregate functions to avoid fetching all rows and aggregating in memory.
  // This is a significant performance win for users with large chat histories.
  ...
  if (!result.length) return { totalSessions: 0, totalMessages: 0, lastActive: null };

  return {
    totalSessions: Number(result[0].totalSessions ?? 0),
    totalMessages: Number(result[0].totalMessages ?? 0),
    lastActive: result[0].lastActive ? new Date(result[0].lastActive) : null,
  };
}
