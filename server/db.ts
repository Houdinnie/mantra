...
export async function getUserStats(userId: number) {
  ...
  // ⚡ Bolt Optimization:
  // Instead of fetching all chat session rows for a user into the Node.js process memory
  // and performing in-memory aggregations/sorting (O(N) time & memory complexity),
  // we delegate the aggregation to the database using SQL aggregate functions: count, sum, max.
  // This reduces memory overhead and improves performance to O(1) in the application layer.
  const [result] = await db
    .select({
