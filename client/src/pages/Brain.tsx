...
export default function BrainPage() {
  ...
  const statusQuery = trpc.brain.status.useQuery(undefined, {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              100,000 neuron neuromorphic system
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {online ? (
            <span className="flex items-center gap-2 text-xs bg-emerald-900/30 border border-emerald-500/30 px-3 py-1.5 rounded-full text-emerald-400">
              ...
              {(ctx as any)?.mode === "neurolinked"
                ? "NeuroLinked"
                : "Embedded"}{" "}
              · {ctx?.state?.development_stage}
            </span>
          ) : (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    {
                      label: "Neurons",
                      value: ctx.state.total_neurons.toLocaleString(),
                    },
                    {
                      label: "Synapses",
                      value: ctx.state.total_synapses.toLocaleString(),
                    },
                    {
                      label: "Memories",
                      value: ctx.state.memory_count.toLocaleString(),
                    },
                    {
                      label: "Uptime",
                      value: `${ctx.state.uptime_hours.toFixed(1)}h`,
                    },
                  ].map(s => (
                    <div
                      key={s.label}
            ...
            {online && nm && (
              <div className="space-y-3">
                ...
                <div className="space-y-3 bg-slate-800/30 border border-slate-700 rounded-lg p-3">
                  <NeuroBar
                    label="Dopamine (learning)"
                    value={nm.dopamine}
                    color="text-yellow-400"
                  />
                  <NeuroBar
                    label="Acetylcholine (attention)"
                    value={nm.acetylcholine}
                    color="text-blue-400"
                  />
                  <NeuroBar
                    label="Norepinephrine (arousal)"
                    value={nm.norepinephrine}
                    color="text-red-400"
                  />
                  <NeuroBar
                    label="Serotonin (calm)"
                    value={nm.serotonin}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ctx.state.active_regions.map(r => (
                    <span
                      key={r}
                      className="text-xs px-2 py-1 rounded-full bg-violet-900/40 border border-violet-500/30 text-violet-300"
                    >
                      {r.replace(/_/g, " ")}
                    </span>
                  ))}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="border-b border-slate-800 p-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={!online || !recallQuery.trim()}
          ...
          <ScrollArea className="flex-1 p-4">
            ...
            {submitted && (
              <div className="mb-6">
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">
                  Recall: "{submitted}" — {recallQuery_.data?.length ?? 0}{" "}
                  results
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {ctx.learned.top_concepts.slice(0, 20).map((c: any) => (
                    <button
                      key={c.concept}
                      onClick={() => {
                        setRecallQuery(c.concept);
                        setSubmitted(c.concept);
                      }}
            ...
            {online && ctx?.insights?.cross_references?.length > 0 && (
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  {ctx.insights.cross_references
                    .slice(0, 5)
                    .map((r: any, i: number) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 p-2 bg-slate-800/30 border border-slate-700 rounded-lg text-sm"
                      >
            ...
            {sleepQuery.data && sleepQuery.data.length > 0 && (
              <div className="mb-6">
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">
                  <Activity className="inline w-3 h-3 mr-1 text-emerald-400" />
                  Sleep Consolidation Insights
                </h3>
                <div className="space-y-2">
                  {sleepQuery.data.map((ins: any, i: number) => (
                    <div
                      key={i}
                      className={`p-3 border rounded-lg ${ins.kind === "cross-reference" ? "bg-violet-900/20 border-violet-500/30" : ins.kind === "pattern" ? "bg-amber-900/20 border-amber-500/30" : "bg-slate-800/40 border-slate-700"}`}
                    >
