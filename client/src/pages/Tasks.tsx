...
const TEMPLATES = [
  {
    ...
    desc: "Daily digest at 7am",
    params: ["channels"],
  },
  {
    id: "urlMonitor",
    label: "🔍 URL Monitor",
    desc: "Watch a URL for changes",
    params: ["url", "checkEveryHours"],
  },
  {
    id: "researchTask",
    label: "📚 Research Task",
    desc: "Deep-research any topic",
    params: ["topic"],
  },
  {
    id: "tradingAlert",
    label: "📊 Trading Alert",
    desc: "Asset price check on schedule",
    params: ["asset"],
  },
];
...
function FlowCard({
  ...
  const triggerEmoji =
    { cron: "⏰", manual: "▶️", webhook: "🔗", message: "💬" }[flow.trigger] ??
    "⚡";
  return (
    <motion.div
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs px-2 py-0.5 rounded-full border ${flow.enabled !== false ? "bg-emerald-900/30 border-emerald-500/30 text-emerald-400" : "bg-slate-800 border-slate-600 text-slate-500"}`}
            >
        ...
        <div className="flex items-center gap-1.5 ml-3 flex-shrink-0">
          <Button
            size="sm"
            variant="ghost"
            onClick={() => flow.id && onTrigger(flow.id)}
...
export default function Tasks() {
  const [tab, setTab] = useState<"flows" | "dispatch" | "ledger" | "deepread">(
    "flows"
  );
  ...
  const statusQuery = trpc.task.status.useQuery(undefined, {
    refetchInterval: 5000,
  });
  const flowsQuery = trpc.task.getFlows.useQuery();
  const ledgerQuery = trpc.task.getLedger.useQuery(
    { limit: 30 },
    { refetchInterval: 3000 }
  );

  const sendMutation = trpc.task.send.useMutation({
    onSuccess: () => setTaskText(""),
  });
  const triggerMutation = trpc.task.triggerFlow.useMutation({
    onSuccess: () => ledgerQuery.refetch(),
  });
  const deleteMutation = trpc.task.deleteFlow.useMutation({
    onSuccess: () => flowsQuery.refetch(),
  });
  const templateMutation = trpc.task.createFromTemplate.useMutation({
    onSuccess: () => {
      flowsQuery.refetch();
  ...
  const deepReadMutation = trpc.task.deepRead.useMutation({
    onSuccess: () => setDeepReadUrls(""),
  });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              OpenClaw Gateway · Flows · DeepReeder
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {online && status && (
            <div className="hidden md:flex items-center gap-4 text-xs text-slate-500">
              ...
              <span>
                {status.activeFlows} active flow
                {status.activeFlows !== 1 ? "s" : ""}
              </span>
      ...
      <div className="flex-1 overflow-hidden">
          ...
          <div className="h-full flex flex-col">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between flex-shrink-0">
              <p className="text-sm text-slate-400">
                {flowsQuery.data?.length ?? 0} automation
                {(flowsQuery.data?.length ?? 0) !== 1 ? "s" : ""} configured
              </p>
              <Button
                size="sm"
                  ...
                  <div className="p-4 bg-slate-900/50">
                    ...
                    {!online && (
                      <p className="text-xs text-amber-400 mt-2">
                        ⚠️ OpenClaw gateway is offline — flows will be created
                        but not executed
                      </p>
                    )}
                  </div>
                ...
                {flowsQuery.data?.length === 0 && !flowsQuery.isLoading && (
                  <div className="text-center py-16">
                    ...
                    <p className="text-slate-600 text-sm">
                      Create a flow from a template above, or use
                      <br />
                      <code className="text-cyan-400">/task</code> in chat to
                      dispatch tasks ad-hoc.
                    </p>
                  </div>
                )}
        ...
        {tab === "dispatch" && (
          <div className="h-full flex flex-col p-6 max-w-2xl">
            ...
            <p className="text-sm text-slate-400 mb-6">
              Send a natural language instruction to your OpenClaw agent. It
              will execute autonomously using its available tools — shell
              commands, browser automation, file ops, messaging.
            </p>
            ...
            <div className="mt-4 flex items-center gap-3">
              <Button
                onClick={() =>
                  taskText.trim() &&
                  sendMutation.mutate({ task: taskText.trim() })
                }
              ...
              {!online && (
                <p className="text-xs text-amber-400">OpenClaw is offline</p>
              )}
              {sendMutation.isSuccess && (
                <p className="text-xs text-emerald-400">
                  ✓ Task dispatched — check Ledger for status
                </p>
              )}
            </div>
          </div>
        ...
        {tab === "deepread" && (
          <div className="h-full flex flex-col p-6 max-w-2xl">
            ...
            <p className="text-sm text-slate-400 mb-6">
              Paste one or more URLs. OpenClaw's DeepReeder skill will scrape
              each one — articles, X/Twitter threads, Reddit posts, YouTube
              transcripts — convert to clean Markdown, and save to agent memory.
              Use <code className="text-cyan-400">/recall</code> in chat to
              surface the content later.
            </p>
            ...
            <div className="mt-4 flex items-center gap-3">
              <Button
                onClick={() => {
                  const urls = deepReadUrls
                    .split(/[\n\s]+/)
                    .filter(u => u.startsWith("http"));
                  if (urls.length) deepReadMutation.mutate({ urls });
                }}
                disabled={
                  !deepReadUrls.trim() || deepReadMutation.isPending || !online
                }
              ...
              {deepReadMutation.isSuccess && (
                <p className="text-xs text-emerald-400">
                  ✓ Queued — content will be saved to agent memory
                </p>
              )}
            </div>
          </div>
            ...
            <ScrollArea className="flex-1">
              <div className="p-4 space-y-2">
                {!online && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-500 text-sm">
                      OpenClaw is offline — no execution history available
                    </p>
                  </div>
                )}
                ...
                {ledgerQuery.data?.map((entry: any) => (
                  <div
                    key={entry.id}
                    className="flex items-start gap-3 p-3 bg-slate-800/40 border border-slate-700 rounded-lg"
                  >
